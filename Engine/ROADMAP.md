...
- [x] **Paragraph breaks on speech pauses** — Implemented via `transcript_formatter.py`: inserts `\n\n` between sentences separated by >0.8 s of silence (0.3 s after terminal punctuation). Uses a `speech_end` field on the Sentence dataclass pointing at the last spoken (non-punct) token's end, because Parakeet's terminal-punct tokens absorb inter-sentence silence and make `sentence.end - sentence.start` always zero. Apple Speech falls through unchanged (no token timings).

- [x] **Audio retention hardening** — "Retain Recordings" toggle removed from the menubar; default is delete-after-transcribe. Developer escape hatch: set `MEETINGNOTES_RETAIN_RECORDINGS=1` in gitignored `Engine/.env.local` (loaded at import time by `environment.py`). Motivated by consent posture for coworker-handoff builds — non-technical users can no longer stumble into a retained recordings folder from the UI.
...
- [ ] **Speaker diarization (who said what)** — Speaker-labelled transcripts (`Speaker 1: ...` / `Speaker 2: ...`) instead of just paragraph breaks on pauses. Real diarization requires a speaker-embedding model + clustering, which is a substantial add: new dep, ~100 MB–1 GB model download in `setup.command`, a new pipeline step, extra pipeline latency. Options roughly lightest to heaviest: sherpa-onnx + speaker embedding (~100 MB, ONNX, fine for 2–4 speakers); custom 3D-Speaker extraction + clustering (smallest footprint, most plumbing); pyannote.audio (most accurate, ~1 GB, heavy dep chain). Only revisit when paragraph-breaks-on-pauses proves insufficient in real use and coworkers specifically ask for named speakers. See Planned → "Paragraph breaks on speech pauses" for the zero-ML precursor.
