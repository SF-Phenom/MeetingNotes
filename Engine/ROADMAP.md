...
- [ ] **Paragraph breaks on speech pauses** — Transcripts are currently a wall of text. Insert `\n\n` between words whose Parakeet timestamps are separated by more than ~800 ms (tune against real transcripts), with a shorter threshold after sentence-terminal punctuation. Gets most of the readability win of true speaker-change detection without any ML: pauses overwhelmingly coincide with speaker yields. Prerequisite: plumb word-level timestamps through `realtime_transcriber.py` → `TranscriptionResult.timestamped_text` (currently a dupe of `plain_text`) → a new `transcript_formatter.py` that does the break insertion before `format_transcript` runs. See ROADMAP "Investigate → speaker diarization" for the heavier version.
